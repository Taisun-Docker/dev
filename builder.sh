...
docker build --no-cache -f ${DOCKER_PATH}Dockerfile.arm32v6 -t ${DOCKERHUB_LIVE}:arm32v6-${TRAVIS_COMMIT}-${DOCKER_TAG} .
docker build --no-cache -f ${DOCKER_PATH}Dockerfile.arm64v8 -t ${DOCKERHUB_LIVE}:arm64v8-${TRAVIS_COMMIT}-${DOCKER_TAG} .
